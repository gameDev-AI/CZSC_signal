...
def download_day_data_by_code(db, code, start_date, end_date):
    # 获取指定股票数据
...
if __name__ == '__main__':
    while True:
        # 建立数据库连接
        db = pymysql.connect(host='127.0.0.1', user='root', passwd='123456', db='baostock_database', charset='utf8')
        ...
        if total != 0:
            ...
            for code in all_stock_codes["code"]:
                ...
                if ("sh.0" not in code) and ("bj.8" not in code)  and ("bj.4" not in code) and ("sh.688" not in code) and ("sz.399" not in code):
                    bars = download_day_data_by_code(db, code, start_dt, end_dt)
                    if bars == []:
                        continue
                    else:
