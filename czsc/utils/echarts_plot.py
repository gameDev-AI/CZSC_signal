...
def kline_pro(kline: List[dict],
    ...
    dz_inside = opts.DataZoomOpts(False, "inside", xaxis_index=[0, 1, 2], range_start=10, range_end=100)
    dz_slider = opts.DataZoomOpts(True, "slider", xaxis_index=[0, 1, 2], pos_top="96%",
                                  pos_bottom="0%", range_start=10, range_end=100)
    ...
    for i, (name, ma) in enumerate(ma_keys.items()):
        is_selected = True if i == 3 else False
    ...
    if zs:
        zs_list1 = zs
        for zs_obj in zs_list1:
            ##竖线
            def get_line1_dts(x):
                return x['sdt'],x['sdt']
            def get_line1_val(x):
                return round(x['zd'], 2),round(x['zg'], 2)
            def get_line2_dts(x):
                return x['sdt'],x['edt']
            def get_line2_val(x):
                return round(x['zd'], 2),round(x['zd'], 2)
            ...
            def get_line4_dts(x):
                return x['sdt'],x['edt']
            def get_line4_val(x):
                return round(x['zg'], 2),round(x['zg'], 2)
                
            def get_line5_dts(x):
                return x['sdt'],x['edt']
                
            def get_line5_val(x):
                return (round(x['zd'], 2)+round(x['zg'], 2))/2, (round(x['zd'], 2)+round(x['zg'], 2))/2
            ...
            zs_dts = list(get_line1_dts(zs_obj))
            zs_val = list(get_line1_val(zs_obj))
            chart_zs = Line()
            chart_zs.add_xaxis(zs_dts)
            chart_zs.add_yaxis(series_name="ZS1", y_axis=zs_val, is_selected=True,
                               symbol="diamond", symbol_size=10, label_opts=label_show_opts,
                               itemstyle_opts=opts.ItemStyleOpts(color="rgba(37, 141, 54, 1.0)", ),
                               linestyle_opts=opts.LineStyleOpts(width=1.5))

            chart_zs.set_global_opts(xaxis_opts=grid0_xaxis_opts, legend_opts=legend_not_show_opts)
            chart_k = chart_k.overlap(chart_zs)
            
            zs_dts = list(get_line2_dts(zs_obj))
            zs_val = list(get_line2_val(zs_obj))
            chart_zs = Line()
            chart_zs.add_xaxis(zs_dts)
            chart_zs.add_yaxis(series_name="ZS2", y_axis=zs_val, is_selected=True,
                               symbol="diamond", symbol_size=10, label_opts=label_show_opts,
                               itemstyle_opts=opts.ItemStyleOpts(color="rgba(37, 141, 54, 1.0)", ),
                               linestyle_opts=opts.LineStyleOpts(width=1.5))
            ...
            zs_dts = list(get_line3_dts(zs_obj))
            zs_val = list(get_line3_val(zs_obj))
            chart_zs = Line()
            chart_zs.add_xaxis(zs_dts)
            chart_zs.add_yaxis(series_name="ZS3", y_axis=zs_val, is_selected=True,
                               symbol="diamond", symbol_size=10, label_opts=label_show_opts,
                               itemstyle_opts=opts.ItemStyleOpts(color="rgba(37, 141, 54, 1.0)", ),
                               linestyle_opts=opts.LineStyleOpts(width=1.5))
            ...
            zs_dts = list(get_line4_dts(zs_obj))
            zs_val = list(get_line4_val(zs_obj))
            
            # print(zs_dts)
            # print(zs_val)
            chart_zs = Line()
            chart_zs.add_xaxis(zs_dts)
            chart_zs.add_yaxis(series_name="ZS4", y_axis=zs_val, is_selected=True,
                               symbol="diamond", symbol_size=10, label_opts=label_show_opts,
                               itemstyle_opts=opts.ItemStyleOpts(color="rgba(37, 141, 54, 1.0)", ),
                               linestyle_opts=opts.LineStyleOpts(width=1.5))
            ...
            zs_dts = list(get_line5_dts(zs_obj))
            zs_val = list(get_line5_val(zs_obj))
            chart_zs = Line()
            chart_zs.add_xaxis(zs_dts)
            chart_zs.add_yaxis(series_name="Zz", y_axis=zs_val, is_selected=True,
                               symbol="diamond", symbol_size=10, label_opts=label_show_opts,
                               itemstyle_opts=opts.ItemStyleOpts(color="#da6ee8"),
                               linestyle_opts=opts.LineStyleOpts(width=1.5))
