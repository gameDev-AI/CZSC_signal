...
from .objects import RawBar, Direction, ZS
...
def check_fx(k1: NewBar, k2: NewBar, k3: NewBar):
    ...
    if k1.high < k2.high > k3.high and k1.low < k2.low > k3.low:
        ...
        fx = FX(symbol=k1.symbol, dt=k2.dt, mark=Mark.G, high=k2.high,
                low=low, fx=k2.high, elements=[k1, k2, k3])

    if k1.low > k2.low < k3.low and k1.high > k2.high < k3.high:
        high = k1.high
        if k1.high < k3.high:
            high = k3.high
        fx = FX(symbol=k1.symbol, dt=k2.dt, mark=Mark.D, high=high,
...
def check_bi(bars: List[NewBar], benchmark: float = None):
    ...
    ab_not_include = (fx_a.high > fx_b.high and fx_a.high > fx_b.low and fx_a.low > fx_b.high and fx_a.low > fx_b.high) \
                 or (fx_b.high > fx_a.high and fx_b.high > fx_a.low and fx_b.low > fx_a.high and fx_b.low > fx_a.high) 
    ...
    # if (ab_not_include) and (len(bars_a) >= min_bi_len or power_enough):
    ...
    if (ab_not_include and len(bars_a) >= min_bi_len):
        fxs_ = [x for x in fxs if fx_a.elements[0].dt <= x.dt <= fx_b.elements[2].dt]
        bi = BI(symbol=fx_a.symbol, fx_a=fx_a, fx_b=fx_b, fxs=fxs_, direction=direction, bars=bars_a)
        ...
        if (bi.direction == Direction.Up and high_ubi > bi.high) \
            ...
            #print(len(bars_a))
            return bi, bars_b
    elif (len(bars_a) >= min_bi_len and power_enough):
        fxs_ = [x for x in fxs if fx_a.elements[0].dt <= x.dt <= fx_b.elements[2].dt]
        bi = BI(symbol=fx_a.symbol, fx_a=fx_a, fx_b=fx_b, fxs=fxs_, direction=direction, bars=bars_a)
...
class CZSC:
    def __init__(self,
        ...
        self.bars_nraw:List[NewBar] = []
        self.bars_ubi: List[NewBar] = []  # 未完成笔的无包含K线序列
        self.fx_list: List[FX] = [] #将所有分型保存
    ...
    def double_check_bi(self):
        if not self.bi_list:
            return
        
        fx_list = check_fxs(self.bars_nraw)
        old_bi_list = self.bi_list
        bi_list = self.bi_list
        bi_len = len(old_bi_list)
        if bi_len > 4:
            for i in range(1, bi_len):
                bi1, bi2 = bi_list[i-1:i+1]
                sdt = bi2.fx_a.dt
                edt = bi2.fx_b.dt
                ...
                if direction == Direction.Down:
                    fxs = [x for x in fx_list if sdt < x.dt < edt]
                    list = [x.fx for x in fxs if x.mark == Mark.D]
                    if list:
                        minvalue = min(list)
                        if minvalue < value:
                            modify_fx = fxs[0]
                            for fx in fxs:
                                if fx.fx == minvalue:
                                    modify_fx = fx
                            fxs_ = [x for x in fx_list if bi1.fx_a.elements[0].dt <= x.dt <= modify_fx.elements[2].dt]
                            bars_a = [x for x in self.bars_nraw if bi1.fx_a.elements[0].dt <= x.dt <= modify_fx.elements[2].dt]
                            newbi1 = BI(symbol=bi1.fx_a.symbol, fx_a=bi1.fx_a, fx_b=modify_fx, fxs=fxs_, direction=bi1.direction, bars=bars_a)
                            
                            fxs_ = [x for x in fx_list if modify_fx.elements[0].dt <= x.dt <= bi2.fx_b.elements[2].dt]
                            bars_a = [x for x in self.bars_nraw if modify_fx.elements[0].dt <= x.dt <= bi2.fx_b.elements[2].dt]
                            newbi2 = BI(symbol=bi1.fx_a.symbol, fx_a=modify_fx, fx_b=bi2.fx_b, fxs=fxs_, direction=bi2.direction, bars=bars_a)
                            old_bi_list[i-1] = newbi1
                            old_bi_list[i+1] = newbi2
                        
                        
                if direction == Direction.Up:
                    fxs = [x for x in fx_list if sdt < x.dt < edt]
                    list = [x.fx for x in fxs if x.mark == Mark.G]
                    if list :
                        maxvalue = max([x.fx for x in fxs if x.mark == Mark.G])
                        if maxvalue > value:
                            modify_fx = fxs[0]
                            for fx in fxs:
                                if fx.fx == maxvalue:
    ...
    def update(self, bar: RawBar):
        ...
        if not self.bars_nraw or bar.dt != self.bars_nraw[-1].dt:
            self.bars_nraw.append(bar)
        else:
            self.bars_nraw[-1] = bar
    ...
    def to_echarts(self, width: str = "1400px", height: str = '580px', bs=None):
        ...
        if len(self.bi_list) > 0:
            ...
            zs_len = len(zs_seq)
            if zs_len > 0:
                zs = [{'sdt': x.sdt, 'edt': x.edt, "zd": x.zd, "zg": x.zg} for x in zs_seq]
        else:
            bi = None
            fx = None
            zs = None
        ...
        chart = kline_pro(kline, bi=bi, fx=fx, width=width, height=height, bs=bs,
                          title="{}-{}".format(self.symbol, self.freq.value), zs=zs)
        return chart
